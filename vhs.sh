...
Where options are: (only the first letter)
	...
	-l(anguage)	ARG		Only encode ARG audio stream (3 letter abrevihation, eg: eng,fre,ger,spa)
...
(BETA)	-p(ass)				Encodes the video with 2-Pass
	...
	-t(imeout)	ARG		Set the timeout between videos to ARG seconds
...
# Suggested audio rates (hz) are around 44'000 to 48'000
# Not yet 'used'
audio_rate=48000
...
#
	...
	while getopts "aBb:c:CDe:hLl:pRr:Sst:Q:vwW" opt
	do 	case $opt in
